...
// @desc    Reschedule a booking to a new future date/time
// @route   PATCH /api/bookings/:id/reschedule
// @access  Private (Participant)
export const rescheduleBooking = async (req, res, next) => {
  try {
    const booking = req.booking || await Booking.findById(req.params.id);
    if (!booking) {
      return res.status(404).json({ success: false, message: 'Booking not found' });
    }
    ...
    if (!newScheduledDate) {
      return res.status(400).json({ success: false, message: 'New scheduled date is required' });
    }

    const parsedDate = new Date(newScheduledDate);
    if (isNaN(parsedDate.getTime()) || parsedDate <= new Date()) {
      return res.status(400).json({ success: false, message: 'New scheduled date must be a valid future time' });
    }

    booking.scheduledDate = parsedDate;
    booking.statusHistory.push({
      status: booking.status,
      note: `Rescheduled to ${parsedDate.toISOString()}. Reason: ${rescheduleReason || 'Customer requested reschedule'}`,
      changedAt: new Date()
    });
